...
public class ItemsManagement_steps {
	...
	public void i_provide_item_information_name_price_unit_and_description(String name, String price, String unit, String description) {
	   itemName	= name + utils.randomNumber();//this method you find it in BrowserUtils file to make name unique 
	...
	public void the_item_is_added_to_the_item_list_table() throws InterruptedException {
		...
		//Scrolling up
		item_page.items_page_table_Name_Column_Header.click();
		utils.scrollUp();
		...
		utils.waitForElementToBeVisible(item_page.items_page_filter_name_box);//we wait for the box
		item_page.items_page_filter_name_box.sendKeys(itemName);//to put any value in the box for ex "coffee mug"
		Thread.sleep(2000);
		
		item_page.items_page_filter_name_box.clear();
		...
		Assert.assertTrue(Driver.getDriver().findElement(By.xpath("//a[contains(text(), '" + itemName +"')]")).isDisplayed());//we could
		//pass any value with itemNmae for ex "coffee mug" or any
	...
	public void i_delete_the_item() throws InterruptedException {
		...
		public void i_click_on_edit_button() {  //to click on the name and then wait for edit and delete options
		   utils.waitUntilElementVisibleWithLocator(By.xpath("//a[contains(text(), '" + itemName +"')]"));
		...
		public void i_update_the_items_price_to(String newPrice) {
		    item_page.items_input_page_price_box.clear();//we use clear() to clear the price in that box
		    item_page.items_input_page_price_box.sendKeys(newPrice);//after we clear the price we send the new price
		}
		
		@When("I click on update item button")
		public void i_click_on_update_item_button() {
		    item_page.items_page_update_item_btn.click();//we click on update button
		}
		//this next step might not work so we need to the to get the full code above that bijan did for me
		...
		public void items_price_should_be_updated_to(String newPrice) {
			...
			String trimmedPrice = newPrice.substring(0, 2);//use this substring() to cut the number 2200 so we can get the 22 only no 00
			System.out.println(trimmedPrice);//to print out the price 
