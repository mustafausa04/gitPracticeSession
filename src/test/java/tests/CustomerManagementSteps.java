...
public class CustomerManagementSteps {
	...
	// @newCustomerBTNTest scenario end -
		...
		public void i_enter_invalid_informatin_display_name_and_email(String name, String email) throws InterruptedException {
			utils.waitForElementToBeVisible(customerPage.customer_page_NewCustomer_TextHeader);//we wait for the page with header text "New customer" on top of the page to be visible 
			customerPage.customer_page_BasicInfo_DisplayName_Field.sendKeys(name);//send the name
			customerPage.customer_page_BasicInfo_Email_Field.sendKeys(email);//send the email
			customerPage.customer_page_NewCustomerSubmit_BTN.click();//hit click
